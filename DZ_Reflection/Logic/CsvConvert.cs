...
public static class CsvConvert
{
    public static string SerializeObject<T>(T obj)
    {
        ...
        oStringBuilder.AppendLine(string.Join(",", objProperties.Select(p => p.Name)));
        oStringBuilder.Append(string.Join(",", objProperties.Select(p => type.GetField(p.Name)!.GetValue(obj))));
    ...
    public static T DeserializeObject<T>(string csvString)
    {
        ...
        if (lines.Length != 2)
        {
            throw new Exception("Неверная строка формата csv");
        }
